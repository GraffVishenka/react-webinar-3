...
export const getTotalPrice = (basket) => {
  ...
  productKeys.forEach((key) => {
    allPrice += basket[key].reduce((acc, cur) => acc + cur.price, 0);
  });

  return productKeys.length ? ` ${allPrice} ₽` : null;
};

// Счётчик товаров в корзину
export const getCount = (basket) => {
  let productCount = 0;
  for (let key in basket) {
    basket[key].length ? (productCount += 1) : null;
  }
